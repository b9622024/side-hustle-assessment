...
export function RadarChart({ profile }: { profile: Profile }) {
  ...
  return (
    <div className="radar-wrap">
      <svg className="radar" viewBox="0 0 200 200" role="img" aria-label="六項副業行動能力雷達圖">
        {[18, 36, 54, 72].map((radius) => <polygon key={radius} points={profile.map((_, index) => point(index, radius)).join(" ")} className="radar-grid" />)}
        {profile.map((_, index) => <line key={index} x1="100" y1="100" x2={point(index, 72).split(",")[0]} y2={point(index, 72).split(",")[1]} className="radar-axis" />)}
        <polygon points={dataPoints} className="radar-shape" />
        {profile.map((item, index) => {
          const [x, y] = point(index, 88).split(",").map(Number);
          return <text key={item.key} x={x} y={y} textAnchor="middle" dominantBaseline="middle" className="radar-label">{item.key}</text>;
        })}
      </svg>
