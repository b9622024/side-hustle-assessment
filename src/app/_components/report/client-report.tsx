...
import { customerReportVisibility as visibility } from "../../../report/customer-report-visibility";
  ...
  return (
    <main className="client-report">
      ...
      <div className="report-grid">
        {visibility.astrology_work_style ? <section className="report-panel">
        ...
        {visibility.numerology ? <section className="report-panel">
        ...
        {visibility.behavior_radar ? <section className="report-panel">
          <SectionTitle number="03" title="副業行動輪廓" description="六項能力呈現你目前最自然的行動資源。" />
          <RadarChart profile={data.actionProfile} />
        </section> : null}

        {visibility.side_hustle_types ? <section className="report-panel">
        ...
        {visibility.frictions ? <section className="report-panel">
          <SectionTitle number="05" title="潛在摩擦風險" description="分數越高，越值得提前設計低阻力的應對方式。" />
          <div className="friction-list">
            {data.frictions.map((item) => <article key={item.key}><div><strong>{item.label}</strong><span>{item.score.toFixed(1)} / 10</span></div><i><b style={{ width: `${item.score * 10}%` }} /></i></article>)}
          </div>
        </section> : null}

        {visibility.execution_spectrums ? <section className="report-panel">
      ...
      <footer className="report-footer"><span>副業羅盤 · Side Hustle Compass</span><span>完整診斷可於一對一解析時進一步了解。本報告用於自我探索與行動規劃，不構成財務或職涯保證。</span></footer>
    </main>
  );
}
