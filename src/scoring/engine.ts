...
import { buildTypeDisplayScores, TYPE_PERCENTILE_METHOD, TYPE_PERCENTILE_REFERENCE_VERSION } from "./type-percentiles";
...
export function scoreAssessment(input:AssessmentInput,_legacyPersonality?:PersonalityScores):ScoringResult {
  ...
  const roundedFinalTypes=mapRound(finalTypes);
  const typeDisplayScores=buildTypeDisplayScores(roundedFinalTypes);
  ...
  const scoringTrace:ScoringTrace={
    ...
    formal_type_precalibrated:mapRound(precalibrated), formal_type_final:roundedFinalTypes,
    type_percentile_method:TYPE_PERCENTILE_METHOD,type_percentile_reference_version:TYPE_PERCENTILE_REFERENCE_VERSION,
  ...
  return {
    ...
    formalBehaviorDimensions:formal.v2,routingDimensions:routing.v2,behaviorTypes:mapRound(precalibrated),finalTypes:roundedFinalTypes,
